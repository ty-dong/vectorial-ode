...
- Use the four elementary functions (sin, cos, exponential and quadratic polynomial function) by setting the ``usual_function`` option in `setting.dat` to 1. During execution, choose one of them as well as set the corresponding parameters for each component. 
- Define $$g$$ in `ODE_Solver/settings/user_defined_g.h`, set the option mentioned above to 0 and recompile the program.
...
$ make
````
An executable file will then be generated:
```
...
## Features
  ...
  - Possible for user to personalize various parameters including paths of read-in data, output data path, initial time, end time, number of discretization steps, etc.
  - Offer four options of common elementary functions for $$g$$ in the right hand side, easily used in terminal and possibility for user to define more complex function in `settings/user_defined_g.h`.
...
- Support more complex function types of $$g$$, eg: $$t*exp(-t), \sin(\pi t)+3t^2$$. Currently if the user wants to use this kind of functions, he has to define his function in a `.h` file and then compile again. 
- Implement implicit solving methods, eg: Backward Euler method. 
- Offer a choice of storing the solution of all steps or only of the end time. Now every step's solution is stored.
